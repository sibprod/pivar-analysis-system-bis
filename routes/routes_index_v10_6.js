...
const tableauPromptEtape1HtmlService = require('../services/visualisation/tableauPromptEtape1HtmlService'); // ⭐ v10.6 (Phase HTML-Etape11-1.0.0)
...
// ═══════════════════════════════════════════════════════════════════════════
// GET /visualiser/etape1/:candidat_id — VISUALISATION HTML ÉTAPE 1.1 INTERNE ⭐ v10.6
// ═══════════════════════════════════════════════════════════════════════════
//
// ⭐ Phase HTML-Etape11 (v10.6 — 2026-05-07)
// Génère dynamiquement le tableau de l'Étape 1.1 (lecture cognitive) d'un
// candidat à partir de la table RESPONSES enrichie par l'agent prompt_etape1.
// Visualisation interne uniquement — NON destinée au candidat ni au DRH.
//
// Usage : https://pivar-analysis-system-bis.onrender.com/visualiser/etape1/<candidat_id>
// Le lien est disponible directement dans Airtable VISITEUR (champ formule
// `lien_visualiser_etape1`).
//
// La page régénère à chaque ouverture → toujours à jour avec les dernières
// données RESPONSES (pas de cache).

router.get('/visualiser/etape1/:candidat_id', async (req, res) => {
  const candidat_id = req.params.candidat_id;
  const startTime = Date.now();

  logger.info('Visualisation Étape 1.1 demandée', { candidat_id, ip: req.ip });
  ...
  try {
    const html = await tableauPromptEtape1HtmlService.generateTableauPromptEtape1Html(candidat_id);
    const elapsed = Date.now() - startTime;
    logger.info('Visualisation Étape 1.1 générée', { candidat_id, elapsedMs: elapsed, htmlSize: html.length });

    res.set('Content-Type', 'text/html; charset=utf-8');
    res.set('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.send(html);
  } catch (error) {
    logger.error('Erreur génération visualisation Étape 1.1', {
      candidat_id, error: error.message, stack: error.stack
    });
    return res.status(500).type('html').send(`
      <html><body style="font-family:sans-serif;padding:40px;text-align:center;">
        <h1>⚠ Erreur lors de la génération du tableau Étape 1.1</h1>
        <p><strong>Candidat :</strong> ${candidat_id}</p>
        <p><strong>Erreur :</strong> ${error.message}</p>
        <p style="color:#888;font-size:11px;">Vérifiez que le candidat existe dans Airtable et qu'il a bien des réponses dans RESPONSES traitées par l'agent prompt_etape1.</p>
      </body></html>
    `);
  }
});
