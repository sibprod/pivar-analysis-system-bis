...
async function run({ candidat_id }) {
  ...
  // 🔒 (garante, 20/07) : le profil (socle/filtre) est IMPÉRATIF au codeur —
  // pour la CONSIGNE du biais uniquement (pare-feu anti-circularité au prompt :
  // les niveaux se codent à l'aveugle du profil). S'il manque, on le crie.
  if (!profil || !profil.socle || !profil.socle.pilier) {
    logger.error('⚠️ PROFIL CANDIDAT MANQUANT au codeur du test — la consigne du biais socle/filtre sera aveugle (violation de protocole à auditer)', { candidat_id });
  }
  const circuits_candidat = await airtableService.getCircuitsTopPourTest(candidat_id).catch((e) => {
    logger.error('⚠️ Circuits candidat non chargés au codeur du test', { candidat_id, error: e.message });
    return [];
  });
