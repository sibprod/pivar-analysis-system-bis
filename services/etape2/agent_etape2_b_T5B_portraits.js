...
//
// v2.2 (2026-07-02) — PROJECTION PAR EXCELLENCE :
//   Chaque sous-agent ne reçoit plus que les champs de SON excellence (conforme à la
//   section ENTRÉE de son prompt), au lieu de la projection complète des 4. Effets :
//   ~60-70 % de tokens d'entrée en moins sur l'agent B (le même gros payload n'est
//   plus facturé 4 fois) et aucun risque de contamination croisée entre excellences.
...
async function run({ candidat_id }) {
  ...
  // v2.2 — Projection PAR EXCELLENCE : chaque sous-agent ne reçoit que les champs
  // de son excellence (+ id_question, numero, scenario). Conforme à la section
  // ENTRÉE de chaque prompt ; réduit le coût d'entrée et évite toute contamination.
  const CHAMPS_EXC = {
    ANT: 'anticipation',
    DEC: 'decentration',
    MET: 'metacognition',
  ...
  function projeter(exc) {
    const f = CHAMPS_EXC[exc];
    return t5aRows.map(r => ({
      id_question: r.id_question || '',
      scenario:    (r.scenario_nom && (r.scenario_nom.name || r.scenario_nom)) || r.scenario || '',
      numero:      r.numero_global || null,
      [exc]: {
        niveau:        val(r[`${f}_niveau`]),
        verbatim:      r[`${f}_verbatim`] || '',
        manifestation: r[`${f}_manifestation`] || ''
      }
    }));
  }

  // Les 4 sous-agents tournent EN PARALLÈLE, chacun son prompt, chacun SA projection,
  // chacun écrit sa ligne. allSettled : un échec isolé ne fait pas tomber les autres.
  const settled = await Promise.allSettled(
    SOUS_AGENTS.map(sa => runSousAgent(sa, candidat_id, projeter(sa.exc)))
  );
