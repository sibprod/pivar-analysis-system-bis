...
//   le statut ETAPE3_TERMINEE (= après l'Agent 3) ou l'un des jalons de reprise.
//
// Machine à états (statut entrant → phase exécutée) :
//   - ETAPE3_TERMINEE / en_cours      → COMPLET   : T5A puis T5BC
//   - ETAPE2_1REPONSE4DIMENSIONS      → reprise   : T5BC seul (T5A déjà fait)
//   - ETAPE2_2EXCELLENCE              → reprise   : T5BC seul (re-agrégation)
//   - ETAPE2_3BILAN4EXCELLENCES       → terminé   : rien à refaire (idempotent)
//
// Jalons posés :
//   T5A écrit   → ETAPE2_1REPONSE4DIMENSIONS
//   T5BC écrit  → ETAPE2_3BILAN4EXCELLENCES, puis ETAPE2_TERMINEE en sortie réussie.
//   (ETAPE2_2EXCELLENCE est un jalon intermédiaire conservé pour la reprise manuelle.)

'use strict';

const airtableService = require('../../infrastructure/airtableService');
const agentT5A        = require('../../agents/etape2/agentT5A');
const agentT5BC       = require('../../agents/etape2/agentT5BC');
  ...
  try {
    ...
    if (!SKIP_T5A_STATUSES.has(statut)) {
      ...
      await airtableService.updateVisiteur(candidat_id, {
        statut_analyse_pivar: 'ETAPE2_1REPONSE4DIMENSIONS',
