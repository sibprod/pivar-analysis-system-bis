...
// Sous-orchestrateur Étape 1 — Profil-Cognitif v10.1
//
...
//
// AIGUILLAGE PAR STATUT (Section 12 du Contrat ETAPE1 v1.7) :
//   - NOUVEAU                  → Pipeline complet (T1 + vérificateur)
//   - REPRENDRE_AGENT1         → Pipeline complet (T1 + vérificateur, T1 supprime/recrée)
//   - REPRENDRE_VERIFICATEUR1  → SAUTE T1, lit ETAPE1_T1 existant, lance UNIQUEMENT vérificateur
//   - en_cours                 → Pipeline complet par défaut (cas marginal)
//
...
async function run({ candidat_id, visiteur }) {
  ...
  // ⭐ v10.1 — Lecture du statut pour aiguillage (Section 12 du contrat v1.7)
  ...
  logger.info('Pipeline Étape 1 starting', {
    ...
    statut_entrant,
    sauterT1,
    mode: sauterT1 ? 'VÉRIFICATEUR_SEUL' : 'PIPELINE_COMPLET'
  });
  logger.info('═══════════════════════════════════════════════════════════', { candidat_id });
