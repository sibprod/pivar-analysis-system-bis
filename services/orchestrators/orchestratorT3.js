...
// ─── Certificateur lexique (dans services/certificateurs/) ─────────────────
// Audit le bilan T4 contre les 15 termes du lexique. Produit audit_certificateur.
// Ajout v10.7.1 (06/05/2026)
...
async function runOrchestratorT4({ candidat_id }) {
  ...
  // ─── 9. Certificateur lexique — audit conformité aux 15 termes ────────
  // ⭐ v10.7.1 (06/05/2026) — Branchement du certificateur.
  // Le certificateur audite le bilan agrégé (56 champs HTML) contre les
  // 15 termes du lexique du protocole. Il produit `audit_certificateur`
  // et une recommandation (ACCEPTER / RETRY_AGENTS_IMPACTES / ESCALADE_HUMAINE).
  //
  // Stratégie T4 v1 : on log la recommandation mais on upserte de toute
  // façon (le retry automatique sera ajouté en T4 v2). Si NON_CONFORME,
  // l'audit_certificateur reste tracé dans ETAPE1_T4_BILAN pour analyse
  // humaine ultérieure.
  let certificateurResult = null;
  try {
    certificateurResult = await certificateurLexiqueService.runCertificateurLexique({
      candidat_id,
      bilan_t4
    });
    trackUsage(usages, costs, certificateurResult);

    logger.info('T4 — Certificateur lexique done', {
      candidat_id,
      verdict:                certificateurResult.verdict,
      score_conformite:       certificateurResult.score_conformite,
      nb_violations_graves:   certificateurResult.nb_violations_graves,
      nb_violations_moyennes: certificateurResult.nb_violations_moyennes,
      recommandation:         certificateurResult.recommandation,
      cost_usd:               certificateurResult.cost.toFixed(4)
    });

    // Injecter audit_certificateur dans le bilan avant upsert
    bilan_t4.audit_certificateur = JSON.stringify({
      verdict:                certificateurResult.verdict,
      score_conformite:       certificateurResult.score_conformite,
      nb_violations_graves:   certificateurResult.nb_violations_graves,
      nb_violations_moyennes: certificateurResult.nb_violations_moyennes,
      nb_violations_mineures: certificateurResult.nb_violations_mineures,
      agents_impactes:        certificateurResult.agents_impactes,
      recommandation:         certificateurResult.recommandation,
      diagnostic_synthese:    certificateurResult.diagnostic_synthese,
      timestamp:              certificateurResult.audit_certificateur
    }, null, 2);

    // Si NON_CONFORME, log un warning. T4 v2 fera retry automatique.
    if (certificateurResult.verdict === 'NON_CONFORME') {
      logger.warn('T4 — Certificateur NON_CONFORME — bilan upserté quand même (T4 v1)', {
        candidat_id,
        agents_impactes: certificateurResult.agents_impactes,
        recommandation:  certificateurResult.recommandation,
        nb_violations:   certificateurResult.violations?.length || 0
      });
    }
  } catch (err) {
    // Le certificateur est non-bloquant en T4 v1 : si l'audit échoue, on
    // upserte le bilan quand même avec un audit_certificateur d'erreur.
    logger.error('T4 — Certificateur lexique failed (non-bloquant)', {
      candidat_id,
      error: err.message
    });
    bilan_t4.audit_certificateur = JSON.stringify({
      verdict: 'ERREUR_AUDIT',
      error:   err.message,
      timestamp: new Date().toISOString()
    });
  }

  // ─── 10. Upsert ETAPE1_T4_BILAN ────────────────────────────────────────
  ...
  logger.info('🎉 Orchestrateur T4 terminé', {
    ...
    certificateur_verdict: certificateurResult?.verdict || 'N/A',
  ...
  return {
    ...
    socle_par_defaut: socleParDefaut,
    filtre,
    certificateur: certificateurResult,
