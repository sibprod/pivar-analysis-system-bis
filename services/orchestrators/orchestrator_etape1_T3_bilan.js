...
//   Le maillon P-C (boucles) est retiré de la chaîne : son service a été supprimé du dépôt
//   (redite avec l'étape 1.2 amont→aval déjà établie). Retraits ici :
//     - require PC supprimé ;
//     - appel PC.run() retiré de _phaseAval ;
//     - case 'REPRENDRE_BILAN_PC' supprimé ;
//     - 'REPRENDRE_BILAN_PC' retiré de STATUTS_BILAN_FABLE.
//   Chaîne désormais : P-A → P-B (filtre) → P-D (marqueurs).
...
const REDACTION   = require('../etape1/etape1_t3/bilan_fable/service_etape1_T3_bilan_PA_pilier');

// Aval inchangé (P-C retiré le 26/06)
...
// ⭐ 12/08/2026 — REJEU FORCÉ DES PILIERS DÉJÀ RÉDIGÉS.
// Par défaut, redigerBilan saute les piliers dont la vue d'ensemble est remplie (reprise sans
// repayer). Pour rejouer un bilan entier — correction d'un service, vérification de
// reproductibilité — il faut le demander explicitement. Variable d'environnement, pour ne pas
// toucher au code à chaque fois : BILAN_PA_FORCE=true.
const BILAN_PA_FORCE =
  String(process.env.BILAN_PA_FORCE || 'false').toLowerCase() === 'true';
...
async function _phasePreparationEtRedaction(candidat_id, civilite) {
  ...
  if (BILAN_PA_FORCE) {
    logger.warn('Rédaction PA — BILAN_PA_FORCE actif : tous les piliers sont rejoués, ' +
                'y compris ceux déjà rédigés', { candidat_id });
  }
  const resultats = await REDACTION.redigerBilan(prep, {
    civilite, write_mode: true, force: BILAN_PA_FORCE
  });
