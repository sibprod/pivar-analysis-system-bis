...
async function callClaude({
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Logger détail erreur si réponse Anthropic disponible
      if (error.response?.data) {
        logger.error('Claude API error body', {
          service,
          errorData: JSON.stringify(error.response.data).substring(0, 1000)
        });
      }
...
async function callClaudeStreaming(body, headers) {
  // Stream SSE — on lit les événements et on reconstitue la réponse
  const axiosResponse = await axios.post(
    claudeConfig.API_URL,
    body,
    {
