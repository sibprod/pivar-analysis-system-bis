...
//
// PHASE v12.2 (2026-08-27) — RÉFÉRENTIELS DES VERSANTS (Mission 1) :
//   ⭐ ADDITIF PUR. Quatre fonctions ajoutées, aucun appelant existant modifié :
//     getReferentielEncadrerManager / getReferentielDimensions (lecture des
//     doctrines E/M et dimensions gravées en base — arbitrage garante
//     27/08/2026) + leurs formateurs pour injection prompt (modèle lexique).
//     Tables par identifiant prouvé, champs par nom prouvé (pièce 48).
//     Consommateur : agentT5C (verdicts des deux faces).
//
  ...
  try {
    ...
    const excellences = t5bRecords.map(r => {
      ...
      return {
        ...
        pattern:         f.pattern || '',
        texte_candidat:  f.texte_candidat || '',
    ...
    try {
      ...
      if (synthTest && synthTest.niveau_global) {
        ...
        // v2.2 (garante, 20/07) : passation ENTAMÉE (certaines réponses posées,
        // pas toutes) = état « reprise » — le bilan doit dire « Reprendre »,
        // jamais « Passer », avec le compte des réponses déjà enregistrées.
        if (rowsTest && rowsTest.length >= 1) {
          const repondues = rowsTest.filter(r => r.response_text && String(r.response_text).trim() !== '').length;
          if (repondues === rowsTest.length) {
            profil.test_dec_statut = 'en_analyse';
          } else if (repondues >= 1) {
            profil.test_dec_statut   = 'reprise';
            profil.test_dec_repondues = repondues;
            profil.test_dec_total     = rowsTest.length;
          }
        }
      }
    } catch (eSynth) {
...
// ═══════════════════════════════════════════════════════════════════════════
// REFERENTIEL_ENCADRER_MANAGER + REFERENTIEL_DIMENSIONS — ⭐ v12.2 (2026-08-27)
// Mission 1 (référentiels des versants) : les doctrines E/M et dimensions
// vivent en base (arbitrage garante 27/08/2026) ; T5C les lit, plus jamais
// de cadrage figé en prompt. Modèle : getReferentielLexique.
// D-AIRTABLE : tables référencées par IDENTIFIANT DE TABLE prouvé (le SDK
// l'accepte comme sélecteur — insensible aux renommages de table) ; les
// CHAMPS se lisent par NOM PROUVÉ (code, contenu — pièce 48 du 27/08/2026).
// ═══════════════════════════════════════════════════════════════════════════
...
async function getReferentielEncadrerManager() {
  try {
    ...
    // ⭐ 11/09/2026 (garante) — DEUX CHAMPS AJOUTÉS AU RÉFÉRENTIEL.
    // « Les référentiels, on les pose dans Airtable : ça les pose officiellement,
    //   et les agents dans les prompts ne peuvent pas les éviter. »
    //   · grille_regimes : la règle de notation opposable (seuils par régime)
    ...
    // Ordre fixe de présentation : ENCADRER puis MANAGER.
    const ordre = ['ENCADRER', 'MANAGER'];
    entrees.sort((a, b) => ordre.indexOf(a.code) - ordre.indexOf(b.code));

    if (entrees.length !== 2) {
      // Doctrine incomplète = on ne produit PAS de verdict sans elle.
      throw new Error(`REFERENTIEL_ENCADRER_MANAGER incomplet : ${entrees.length}/2 entrées lisibles (code+contenu)`);
    }

    logger.debug('Référentiel encadrer/manager loaded', { count: entrees.length });
    return entrees;
  } catch (error) {
    logger.error('Failed to load REFERENTIEL_ENCADRER_MANAGER', { error: error.message });
    throw error;
  }
}

async function getReferentielDimensions() {
  try {
    const records = await getBase()(TABLE_ID_REFERENTIEL_DIMENSIONS)
      .select()
      .all();
    ...
    // Ordre fixe de présentation : ANT · VUE · DEC · MET.
    const ordre = ['ANT', 'VUE', 'DEC', 'MET'];
    entrees.sort((a, b) => ordre.indexOf(a.code) - ordre.indexOf(b.code));

    if (entrees.length !== 4) {
      throw new Error(`REFERENTIEL_DIMENSIONS incomplet : ${entrees.length}/4 entrées lisibles (code+contenu)`);
    }

    logger.debug('Référentiel dimensions loaded', { count: entrees.length });
    return entrees;
  } catch (error) {
    logger.error('Failed to load REFERENTIEL_DIMENSIONS', { error: error.message });
    throw error;
  }
}

function formaterEncadrerManagerPourPrompt(entrees) {
  const lignes = ['# 🔒 RÉFÉRENTIEL DES DEUX FACES DU MÉTIER (source : Airtable REFERENTIEL_ENCADRER_MANAGER) — NON NÉGOCIABLE', ''];
...
function formaterDimensionsPourPrompt(entrees) {
  const lignes = ['# 🔒 RÉFÉRENTIEL DES QUATRE DIMENSIONS (source : Airtable REFERENTIEL_DIMENSIONS) — NON NÉGOCIABLE', ''];
  ...
  lignes.push('---');
  lignes.push("**RÈGLE ABSOLUE** : définitions, pièges de sur-cotation, régimes épistémologiques et versants font foi, mot pour mot. Toute reformulation est interdite.");
...
async function getReferentielCircuits() {
  try {
    ...
    const circuits = records.map(r => ({
      ...
      // ⚠ CT-025 (12/08/2026) : « circuit_id » est une liste déroulante — Airtable
      // renvoie { color, id, name }. Sans extraction, l'index des circuits officiels
      // contiendrait des objets et la comparaison à la chaîne « C4 » échouerait
      // pour TOUS les circuits, pas seulement ceux de P5.
      circuit_id:  extractLookup(r.fields.circuit_id),
...
// ⚠ CT-025 (11/08/2026) : cette fonction s'appelait getReferentielCircuits et
// écrasait, par collision de noms, celle de la ligne ~1261 qui rend les 75
// circuits. Tous les appelants recevaient 60 circuits, sans P5. Renommée le
// 12/08/2026 — le cache est renommé avec elle, sinon il resservirait le
// référentiel amputé après correction.
let _refCircuitsPaletteCache = null;
async function getReferentielCircuitsPalette() {
  if (_refCircuitsPaletteCache) return _refCircuitsPaletteCache;
  try {
    const records = await getBase()('REFERENTIEL_CIRCUITS').select({}).all();
    _refCircuitsPaletteCache = records
      .map(r => ({
      ...
      .filter(c => c.pilier && c.pilier !== 'P5');
    return _refCircuitsPaletteCache;
  } catch (error) {
    logger.error('Failed to get référentiel circuits (palette)', { error: error.message });
    throw error;
  }
}
...
async function getDejaDitEtape1(candidat_id) {
  try {
    const records = await getBase()('ETAPE1_T3_PILIER')
      .select({ filterByFormula: `{candidat_id} = "${candidat_id}"` })
      .all();
    const profil_etape1 = {}, deja_dit_etape1 = {};
    for (const r of records) {
      const p = (r.fields.pilier && (r.fields.pilier.name || r.fields.pilier)) || '';
      if (!p) continue;
      profil_etape1[p]   = r.fields.pilier_mode || '';
      deja_dit_etape1[p] = r.fields.synth_interpretee || '';
    }
    return { profil_etape1, deja_dit_etape1 };
  } catch (error) {
    logger.error('Failed to get déjà-dit Étape 1', { candidat_id, error: error.message });
    throw error;
  }
}
...
module.exports = {
  ...
  getReferentielCircuitsPalette,   // ⭐ CT-025 — ex-getReferentielCircuits (P1-P4)
