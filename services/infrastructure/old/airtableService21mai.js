...
// ═══════════════════════════════════════════════════════════════════════════
// RESPONSES — patchResponseRow (Phase ETAPE1.1-1.0.0 v10.6, 2026-05-07)
// ═══════════════════════════════════════════════════════════════════════════
//
// Patche UN record RESPONSES avec les 10 champs de sortie de l'étape 1.1
// (lecture cognitive). Utilisé par agentPromptEtape1Service.
//
// Champs attendus dans `fields` (tous optionnels mais doivent suivre la doctrine
// du prompt prompt_etape1 v3.1bis) :
//   - v2_repond_question, v2_repond_situation, v2_analyse
//   - cog_comprend, cog_outils_mobilises
//   - cog_pilier_sortie, cog_sortie_commentaire
//   - cog_pilier_gouverne, cog_gouverne_commentaire, cog_resultat_vise

async function patchResponseRow(recordId, fields) {
  if (!recordId) {
    throw new Error('patchResponseRow — recordId est requis');
  }
  if (!fields || typeof fields !== 'object') {
    throw new Error('patchResponseRow — fields doit être un objet');
  }
...
module.exports = {
  ...
  // RESPONSES (lecture + patch)
  getResponses,
  getResponsesBySession,
  patchResponseRow,         // ⭐ v10.6 (Phase ETAPE1.1)
