...
async function runAgentT3Bilan({ candidat_id, visiteur = null }) {
  ...
  const architecture = computeArchitecture(sources.ventilationPiliers, sources.inventaireCircuits);
...
/**
 * Calcule l'architecture moteur — modèle AMONT / SOCLE / AVAL / FONCTIONNELS.
 *
 * DOCTRINE (Isabelle, 03/06/2026) :
 *   - Cycle cognitif naturel : P1 collecte < P2 tri < P3 analyse < P4 création < P5 exécution.
 *     amont = pilier AVANT le socle dans ce cycle ; aval = pilier APRÈS le socle.
 *   - VUE 1 (AMONT) : sur les circuits DU SOCLE (pilier_owner = socle), regarder vers quel pilier
 *     AVANT le socle ils vont (colonnes nb_svc_<pilier>). Le pilier qui RESSORT NETTEMENT = amont.
 *     Sinon : pas d'amont (socle autonome — c'est une signature, on ne force jamais).
 *   - VUE 2 (AVAL) : sur TOUS les circuits, colonne nb_svc_<socle>. Le pilier APRÈS le socle qui
 *     est le plus au service du socle = aval (la sortie / finition).
 *   - le reste = fonctionnels.
 *   - Le chiffre POSE l'ordre indicatif ; l'agent CONFIRME via le fond (boucles) ; le vérificateur
 *     contrôle la cohérence prose <-> architecture.
 *   Pas de seuil en % : sélection RELATIVE (un pilier se détache des autres) + plancher de matérialité
 *   (un amont doit être cherché sur plusieurs circuits du socle, pas 1-2 miettes isolées).
 */
const ORDRE_CYCLE = { P1: 1, P2: 2, P3: 3, P4: 4, P5: 5 };
// Plancher anti-miettes : nb minimal de circuits du socle allant vers un pilier pour le retenir comme
// amont (un pattern réel, pas du bruit). PAS un seuil métier en % ; un minimum de matérialité.
const MATERIALITE_MIN_CIRCUITS = 3;

function computeArchitecture(ventilationPiliers, inventaireCircuits) {
  const PILIER_LABELS = {
  ...
  const inv = Array.isArray(inventaireCircuits) ? inventaireCircuits : [];

  // ─── 1. SOCLE = pilier rang_par_frequence=1 (sinon max nb_reponses) ──
  ...
  if (!socleRow) return { ordered: [], socle: null, amont: null, aval: null };
  const soclePilier = socleRow.pilier_coeur;
  const socleNum = ORDRE_CYCLE[soclePilier] || 0;

  const svcVers = (circ, pil) => Number(circ[`nb_svc_${pil}`] || 0);
  const ownerOf = (circ) => circ.pilier_owner || circ.pilier ||
    ((circ.circuit_label || '').match(/^P\d/) || [''])[0];

  const coeurDe = {};
  for (const v of ventilationPiliers) {
    coeurDe[v.pilier_coeur] = Number(v.nb_activations_coeur_total || 0);
  }

  // ─── 2. VUE 1 (AMONT) : circuits owner=socle → piliers AVANT le socle ──
  const circuitsSocle = inv.filter(c => ownerOf(c) === soclePilier);
  const vue1 = {};
  for (const p of Object.keys(ORDRE_CYCLE)) {
    if (ORDRE_CYCLE[p] >= socleNum) continue;
    let circuits = 0, occ = 0;
    for (const c of circuitsSocle) {
      const s = svcVers(c, p);
      if (s > 0) { circuits += 1; occ += s; }
    }
    vue1[p] = { circuits, occ };
  }
  let amont = null;
  const amontTri = Object.keys(vue1).sort((a, b) =>
    (vue1[b].circuits - vue1[a].circuits) || (vue1[b].occ - vue1[a].occ));
  if (amontTri.length) {
    const c1 = vue1[amontTri[0]];
    const c2 = amontTri[1] ? vue1[amontTri[1]] : { circuits: 0, occ: 0 };
    const seDetache = (c1.circuits > c2.circuits) ||
      (c1.circuits === c2.circuits && c1.occ > c2.occ && c1.circuits > 0);
    if (c1.circuits >= MATERIALITE_MIN_CIRCUITS && seDetache) amont = amontTri[0];
  }

  // ─── 3. VUE 2 (AVAL) : tous circuits → nb_svc_<socle>, piliers APRÈS le socle ──
  const vue2 = {};
  for (const p of Object.keys(ORDRE_CYCLE)) {
    if (ORDRE_CYCLE[p] <= socleNum) continue;
    let circuits = 0, occ = 0;
    for (const c of inv) {
      if (ownerOf(c) !== p) continue;
      const s = svcVers(c, soclePilier);
      if (s > 0) { circuits += 1; occ += s; }
    }
    vue2[p] = { circuits, occ };
  }
  let aval = null;
  const avalTri = Object.keys(vue2).sort((a, b) =>
    (vue2[b].occ - vue2[a].occ) || (vue2[b].circuits - vue2[a].circuits));
  if (avalTri.length && vue2[avalTri[0]].occ > 0) aval = avalTri[0];

  // ─── 4. Construire l'architecture ──
  const socleLabel = socleRow.pilier_coeur_libelle || PILIER_LABELS[soclePilier] || soclePilier;
  const ordered = [{
    pilier: soclePilier, pilier_label: socleLabel, role: 'socle',
    coeur: coeurDe[soclePilier] || 0, vase_clos: false, raw: socleRow
  }];
  const pushPilier = (p, role) => {
    if (!p) return;
    ordered.push({
      pilier: p, pilier_label: PILIER_LABELS[p] || p, role,
      coeur: coeurDe[p] || 0,
      svc_au_socle: (vue2[p] && vue2[p].occ) || 0,
      socle_va_chercher: (vue1[p] && vue1[p].occ) || 0,
      raw: ventilationPiliers.find(v => v.pilier_coeur === p) || { pilier_coeur: p }
    });
  };
  if (amont) pushPilier(amont, 'amont');
  if (aval) pushPilier(aval, 'aval');
  for (const p of Object.keys(ORDRE_CYCLE)) {
    if (p === soclePilier || p === amont || p === aval) continue;
    pushPilier(p, 'fonctionnel');
  }

  return { ordered, socle: ordered[0], amont: amont || null, aval: aval || null, _vue1: vue1, _vue2: vue2 };
}
...
function buildPayloadAppel0bis(candidat_id, sources, architecture) {
  return {
    ...
    architecture_moteur: architecture.ordered.map(p => ({
      pilier: p.pilier,
      role: p.role,
      coeur: (p.coeur != null ? p.coeur : null),
      svc_au_socle: (p.svc_au_socle != null ? p.svc_au_socle : null),
      socle_va_chercher: (p.socle_va_chercher != null ? p.socle_va_chercher : null),
    })),
...
function buildPayloadAppelPilier(candidat_id, sources, architecture, pilierArch) {
  ...
  return {
    ...
    compteurs_pilier: {
      ...
      svc_au_socle:         (pilierArchInfo.svc_au_socle != null ? pilierArchInfo.svc_au_socle : null),
      socle_va_chercher:    (pilierArchInfo.socle_va_chercher != null ? pilierArchInfo.socle_va_chercher : null),
    },
...
const ROLE_LABELS = {
  ...
  aval:            'Pilier aval — Ce par quoi le socle conclut et agit',
  fonctionnel:     'Pilier fonctionnel',
  // anciens libellés conservés pour rétro-compatibilité (records antérieurs)
