...
async function lireStructureDepuisPourbilan(candidat_id) {
  ...
  // echelle_classement : VESTIGE (v12.1). Le rangement se fait TOUJOURS sur le total
  // (doctrine des blocs figée 23/06). On ne calcule plus de bascule coeur/total : ce
  // champ est conservé pour compatibilité de structure mais vaut toujours "total" et
  // ne commande plus rien (le prompt le marque IGNORÉ).
  const piliers = ordrePiliers.map(code => {
    const p = parPilier[code];
    return { ...p, echelle_classement: 'total' };
  });
