...
const F_BILAN = {
  ...
  gestes:            'fldQr5PWbmaTH2uwv',   // ⭐ v4 (02/07) — filtre_gestes : JSON des gestes décodés 3 strates
  synthese:          'fldzYiMkf7HgdBddj',   // ⭐ v4 (02/07) — filtre_synthese : pose du filtre après les gestes
};
...
async function lireBlocHautSocle(cid, socle, archSocle){
  ...
  for(const r of tous){
    ...
    circuitsByCode[full]={
      ...
      coeur: _num(r.circuit_freq),        // ⭐ v4 — cœur (nb activations gouvernées) pour "17 fois"
      decodage: r.n3_nuance || '',        // ⭐ v4 — décodage validé à découper en fait/revele
      verbatims: verbs,
    };
  }
  ...
  if(codes.length){
    circuits = codes.map(code=>{
      const c=circuitsByCode[code]||{code,libelle:'',profondeur:'',total:0,coeur:0,decodage:'',verbatims:[]};
...
function construireEntree(cid, socle, presentation, blocHaut, instr, pa_socle){
  return {
    ...
    bloc_haut_socle: { nom_bloc: blocHaut.nom_bloc, circuits: blocHaut.circuits.map(c=>({
      ...
      total:c.total, coeur:c.coeur, decodage:c.decodage, verbatims:c.verbatims,
    })) },
...
async function ecrire(cid, sortie, analyse){
  ...
  if(!Array.isArray(sortie.gestes) || sortie.gestes.length === 0)
    throw new Error('Sortie agent : "gestes" vide — le décodage 3 strates n\'a pas été produit');
  const fields={
    ...
    [F_BILAN.gestes]:            JSON.stringify(sortie.gestes),   // ⭐ v4 — gestes décodés (3 strates)
    [F_BILAN.synthese]:          sortie.synthese||'',             // ⭐ v4 — synthèse (pose du filtre après)
  };
