...
async function appellerClaude(entree, opts = {}, erreursPrecedentes = null) {
  ...
  // ⭐ 12/08/2026 — REPRISE INFORMÉE.
  // Jusqu'ici la reprise renvoyait la MÊME entrée sans dire ce qui n'allait pas : l'agent
  // ignorait quel champ dépassait et de combien, et la tentative 2 pouvait être pire que la 1
  // (cas P4 du 12/08 : 2 erreurs puis 4). On lui transmet désormais la liste des manquements.
  // Le message d'erreur est ajouté APRÈS l'entrée, en second tour de conversation, pour ne
  // pas modifier le contenu mis en cache.
  const messages = [{ role: 'user', content: JSON.stringify(entree, null, 2) }];
  if (erreursPrecedentes && erreursPrecedentes.length) {
    messages.push({ role: 'assistant', content: '(réponse précédente)' });
    messages.push({
      role: 'user',
      content:
        'Ta réponse précédente a été rejetée par la validation. Manquements relevés :\n' +
        erreursPrecedentes.map(e => '  - ' + e).join('\n') +
        '\n\nReprends la MÊME analyse et le MÊME rangement : ne rejuge ni la profondeur, ni la ' +
        'cassure, ni les blocs. Corrige UNIQUEMENT les champs listés ci-dessus, en respectant ' +
        'les plafonds (explication_courte ≤ 18 mots, soleil_micro ≤ 15 mots, intro_eclate ≤ 40 ' +
        'mots). Reproduis la réponse complète au même format : le bloc <analyse> puis le JSON.',
    });
  }
...
async function redigerPilier(prep, pilier, refs, opts) {
  ...
  const MAX_RETRY = opts.max_retries || 3;
  let pa, analyse = '', validation, erreursPrecedentes = null;
  for (let t = 1; t <= MAX_RETRY; t++) {
    ({ pa, analyse } = await appellerClaude(entree, opts, erreursPrecedentes));
    validation = valider(pa, entree, analyse);
    if (validation.ok) {
      if (t > 1) logger.info(`Rédaction PA ${pilier.pilier_code} — validée à la tentative ${t}/${MAX_RETRY}`);
      break;
    }
    logger.warn(`Rédaction PA ${pilier.pilier_code} — ${validation.errors.length} erreur(s), tentative ${t}/${MAX_RETRY}`,
      { errors: validation.errors });
    erreursPrecedentes = validation.errors;
    if (t === MAX_RETRY && opts.strict) throw new Error(`Validation PA échouée (${pilier.pilier_code})`);
  }

  // ⭐ 12/08/2026 — CONSTAT DE NON-CONFORMITÉ.
  // Hors mode strict, la dernière réponse est conservée même si elle viole encore les règles.
  // Ce fait était noyé dans un WARN de tentative ; il est désormais tracé explicitement,
  // avec le détail, pour que la sortie retenue ne passe pas pour conforme.
  if (!validation.ok) {
    logger.error(
      `Rédaction PA ${pilier.pilier_code} — SORTIE RETENUE NON CONFORME après ${MAX_RETRY} tentatives`,
      { candidat_id: prep.candidat_id, nb_erreurs: validation.errors.length,
        errors: validation.errors });
  }
