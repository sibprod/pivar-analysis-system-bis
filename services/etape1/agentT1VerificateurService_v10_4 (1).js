...
//      le vérificateur v1.3 puisse les voir et les vérifier. Sans cet
...
//
//   Le vérificateur correspondant est verificateur1_t1.txt v1.3, qui
//   contient la matrice des pièges v2 dérivée de l'audit Cécile/Rémi/Véronique.
...
async function runVerificateurT1({ candidat_id, rows_t1, tentatives_actuelles = 0 }) {
  ...
  // Construction du payload conforme au prompt verificateur1_t1.txt v1.3 (section 3)
  ...
  //           Sans cela, le vérificateur v1.3 ne pourrait pas vérifier la
  ...
  const payload = {
    ...
    lignes_t1: rowsWithIds.map(row => ({
      ...
      // Sans ces champs dans le payload, le vérificateur v1.3 ne peut pas
