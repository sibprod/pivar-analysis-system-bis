// services/visualisation/service_etape1_T3_bilan_payload_def.js
// Payload service — Bilan Fable cognitif complet — VERSION DÉFINITIVE (tableau3circuitspourbilandef)
// v2.0-def — 26/06/2026
//
// IDENTIQUE au payload tableau2circuitspourbilan, SAUF 2 enrichissements (route def) :
//   1. BLOC : groupé sur le champ RÉEL `bloc_final` (très souvent/souvent/occasionnels)
//      attribué par l'agent bilan-pilier, au lieu du regroupement par seuil de total.
//   2. PROFONDEUR : lue depuis POURBILAN `profondeur_attribuee` (remplie par le maillon),
//      repli T3_CIRCUIT si vide.
// Repli intégral sur l'ancien comportement si bloc_final/profondeur_attribuee absents.
//
...
// ─────────────────────────────────────────────────────────────────────────────
// RANGEMENT EN BLOCS — VERSION DÉFINITIVE (26/06/2026)
// Cette version def lit le bloc RÉEL attribué par l'agent bilan-pilier, écrit dans
// le champ POURBILAN `bloc_final` (très souvent / souvent / occasionnels) par le maillon.
//   - bloc_final renseigné → on l'affiche tel quel (le rangement de l'agent fait foi).
//   - bloc_final vide (candidat pas encore repassé par l'agent) → REPLI sur l'ancien
//     comportement : champ `bloc` d'origine, sinon déduction par le total. Aucune page cassée.
// ─────────────────────────────────────────────────────────────────────────────
const BLOC_EN_ATTENTE = 'en attente d\'attribution de bloc';
// Ordre d'affichage des blocs réels : très souvent → souvent → occasionnels (puis en attente en repli).
const ORDRE_BLOCS = ['très souvent', 'souvent', 'occasionnels', BLOC_EN_ATTENTE];
function blocRank(b) { const i = ORDRE_BLOCS.indexOf(safeStr(b).toLowerCase()); return i < 0 ? 99 : i; }
// Bloc d'affichage d'un circuit (DEF) : bloc_final réel, sinon repli (bloc d'origine / total).
function blocAffichage(r) {
  const bf = safeStr(r.bloc_final).toLowerCase();
  if (bf === 'très souvent' || bf === 'souvent' || bf === 'occasionnels') return bf;
  // — repli si bloc_final absent —
...
async function buildPayload(candidat_id) {
  ...
  for (const po of piliersOrdonnes) {
    ...
    for (const r of circuits) {
      ...
      // DEF : profondeur lue depuis POURBILAN (profondeur_attribuee, remplie par le maillon),
      // avec repli sur T3_CIRCUIT (detailByCode) si le champ POURBILAN est encore vide.
      const profondeur = safeStr(r.profondeur_attribuee) || safeStr(detailByCode[codeAff]?.profondeur);
