...
// Orchestrateur Profil-Cognitif v9.0 — Pipeline complet Étape 1
//
// Pipeline :
//   T1 → T2 → T3 → [T4-Architecture, T4-Circuits, T4-Mode, T4-Transverses en parallèle]
//      → T4-Synthèse → T4-Coûts → Certificateur
//
 ...
 * @returns {Promise<Object>} { success, candidat_id, totalCostUsd, totalElapsedMs, certifReport }
 */
async function processCandidate(session_id) {
  ...
  logger.info('Pipeline v9 starting', { candidat_id });
  logger.info('═══════════════════════════════════════════════════════════', { candidat_id });
  ...
  try {
    ...
    logger.info('✅ T1 done', { candidat_id, rows: t1Result.rows.length, cost: t1Result.cost.toFixed(4) });
    ...
    logger.info('✅ Certificateur done', {
      candidat_id,
    ...
    logger.info('═══════════════════════════════════════════════════════════', { candidat_id });
    logger.info('🎉 Pipeline v9 completed', {
      candidat_id,
      totalElapsedMs,
    ...
    logger.error('Pipeline v9 failed', {
      candidat_id,
      error: error.message,
