...
async function getResponses(session_id) {
  try {
    const records = await base(airtableConfig.TABLES.RESPONSES)
      .select({
        filterByFormula: `{session_ID} = "${session_id}"`,
...
async function getBilan(session_id) {
  try {
    const records = await base(airtableConfig.TABLES.BILAN)
      .select({
        filterByFormula: `{session_ID} = "${session_id}"`,
        maxRecords: 1
      })
...
async function getGlobal(session_id) {
  try {
    ...
    const records = await base(airtableConfig.TABLES.GLOBAL)
...
async function createOrUpdateGlobal(session_id, fields) {
  try {
    ...
    if (existing) {
      await base(airtableConfig.TABLES.GLOBAL).update([{ id: existing.id, fields }]);
    } else {
      await base(airtableConfig.TABLES.GLOBAL).create([{ fields: { session_ID: session_id, ...fields } }]);
    }
  } catch (error) {
