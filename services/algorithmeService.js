...
async function run(session_id, candidat, questionsData, agent3Syntheses, agent1Corpus) {
  ...
  const bilanFields = {
    ...
    // Champs synthèse globale — colonnes dédiées BILAN
    niveau_global:       niveauGlobal.niveau,
    zone_globale:        niveauGlobal.zone,
    score_global:        round2(scoreGlobal),
    profil_type:         profilType,
    distribution_reelle: JSON.stringify(Object.fromEntries(PILIERS.map(p => [p, { nb_questions: dist[p].length, ecart: (ecarts[p] >= 0 ? '+' : '') + ecarts[p] }]))),
    pattern_emergent:    agent1Corpus?.section_C?.pattern_emergent || agent1Corpus?.pattern_emergent || null,
