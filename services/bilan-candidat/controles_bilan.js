...
const LIAISON = new Set(['votre','vos','le','la','les','un','une','des','de','du','d','l','en','et',
  'à','au','aux','ce','cet','cette','qui','que','pour','sans','avec','sur','dans','par','plus','ne','pas','se','son','sa','ses']);
...
function controleTitres(sortie, payload) {
  ...
  for (const o of payload.outils) {
    const g = o.gestes;
    if (Array.isArray(g)) g.forEach(x => matiereParCode.set(x.code, [x.narration, JSON.stringify(x.verbatims), x.revele].join(' ')));
  ...
  for (const t of (sortie.titres_parles||[])) {
    ...
    const orphelins = normalise(t.titre).filter(m => !LIAISON.has(m) && !matiere.has(m));
