...
function mapToAirtableFields(parsed) {
  ...
  return {
    ...
    // pilier_reponse_coeur_confirme SUPPRIMÉ — doublon de pilier_reponse_coeur (même valeur, décision session)
    justification_attribution_pilier_coeur: arbitrage.justification_arbitrage || null,
    piliers_actives_final:              JSON.stringify(parsed.piliers_actives || []), // RENOMMÉ : contient des PILIERS (P1,P3...) pas des circuits
...
async function run(session_id, questions, agent1Analyses) {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    // JSON complet du vérificateur dans son champ dédié
