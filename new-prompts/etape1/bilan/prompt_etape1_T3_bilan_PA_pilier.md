...
Ajouts majeurs sur la version 9 (refonte capacité + profondeur) :
  ...
  Δ-ECH   DOCTRINE DE CLASSEMENT (rectifiée 23/06/2026) : on RANGE sur le TOTAL
          ...
          la cassure SUR LE FOND (libellé + capacité + verbatims), comme le mode. On
          ANALYSE sur les deux chiffres (le cœur reste lu, il dit ce qui gouverne), on
          COMMENTE l'écart cœur/total quand il est marqué.
...
───────────────────────────────────────────────────────────────────────
DOCTRINE DE CLASSEMENT — RANGER SUR LE TOTAL, ATTRIBUER LES BLOCS PAR JUGEMENT DE FOND (figée 23/06, méthode précisée 25/06)
───────────────────────────────────────────────────────────────────────
...
3) LA CASSURE SE JUGE SUR LE FOND, COMME LE MODE (jamais sur le seul chiffre).
   ...
   lecture du GESTE RÉEL, circuit par circuit.

   Pour chaque circuit de total ≥ 3, tu lis trois choses ensemble :
     – son LIBELLÉ (le nom officiel — il situe la catégorie, mais ne dit pas ce que le
       candidat fait concrètement) ;
     – sa CAPACITÉ (le terme exact fourni dans l'entrée : "simple" / "simple avec
       détails-variantes" / "plusieurs simples distinctes" / "sophistiquée" / "plusieurs
       sophistiquées distinctes") ;
     – les VERBATIMS de chaque réponse répertoriée pour ce circuit — c'est LÀ qu'est le
       geste réel du candidat, ce qu'il fait concrètement. Le libellé et la capacité ne
       font que cadrer ; le verbatim porte le geste.

   Tu fais cette lecture pour chaque circuit, l'un après l'autre. Puis tu regardes
   l'ensemble : les verbatims des circuits de tête montrent-ils UN MÊME GESTE D'ENSEMBLE
   (les circuits se combinent, le candidat les active ensemble dans la même démarche), ou
   un sous-groupe montre-t-il un geste D'UNE AUTRE NATURE (catégorie de capacité différente,
   démarche que les verbatims montrent à part) ? La cassure tombe là où le geste change de
   nature, PAS où le chiffre baisse d'une unité.

   PROFIL COMPACT — LE CAS DU PLATEAU D'ACTIVATION.
   Certains candidats ont, sur un pilier, plusieurs circuits de capacité "sophistiquée" ou
   "plusieurs sophistiquées distinctes" dont le total colle au cœur (peu ou pas de service
   rendu aux autres piliers). Leurs totaux forment une pente douce sans rupture franche.
   Quand les verbatims montrent que ces circuits SE COMBINENT DANS UNE MÊME DÉMARCHE — le
   candidat les active tous, à chaque fois, dans le même mouvement, les gestes s'appelant les
   uns les autres au sein de la même réponse — il n'y a pas de cassure : c'est un PLATEAU.
   Tous ces circuits forment UN SEUL bloc « très souvent », sans « souvent ». On ne coupe pas
   en deux un geste d'ensemble.

   → NE FORCE JAMAIS UNE CASSURE QUI N'EXISTE PAS. Le nombre de blocs est VARIABLE : un pilier
     peut n'avoir que « très souvent » + « occasionnels », sans « souvent ». La structure
     épouse le candidat, on ne le range pas dans des cases qui ne sont pas les siennes.
...
4) SÉQUENCE D'ANALYSE (détail des TEMPS 1→3 du CONDUCTEUR ; c'est ce qui te permet de juger la cassure) :
   ...
   e. GARDE-FOU INTANGIBILITÉ : le "total" et le "coeur" de chaque circuit viennent de
      ...
      circuit est à total 4, il est cité à 4, quel que soit son bloc.
...
ÉTALON v10 — Rémi P4C15 (cœur 3, total 5, socle ; capacité "plusieurs sophistiquées distinctes" ; profondeur plein régime) :
...
geste le plus fréquent en Création de solutions (total 5), et vous le menez jusqu'au bout : le
...
pleinement déployé."
  ...
  Note écart : cœur 3 / total 5 → écart 2 (non marqué) → ne PAS commenter l'écart. Le geste reste
...
CONTENU OBLIGATOIRE :
  1. Liste des circuits avec codes, cœur, total, et distribution instrumentale (svc Px).
     Les chiffres cœur et total sont cités EXACTEMENT comme dans l'entrée (intangibilité).
...
la cassure SUR LE FOND (libellé + capacité + verbatims, comme le mode). Le chiffre cité en
premier reste le CŒUR (ce qui gouverne), suivi du total. Cassure = jugement de lecture : on
s'arrête là où le geste change de nature, pas où le chiffre baisse.
...
CAS PLATEAU PARTIEL — Véronique P3 (un groupe de tête se détache, puis un plateau continu) :
...
n'est nette. C'est l'illustration de « ne pas forcer une cassure qui n'existe pas » — ici le plateau
est PARTIEL (un groupe de tête + un plateau en dessous).

CAS PLATEAU TOTAL — Rémi P4 (socle, profil compact ; tout le pilier est un seul plateau) :
Totaux : P4C15=5, P4C8=4, puis P4C1, P4C4, P4C9, P4C10 = 3 (P4C11=2 → occasionnels, posé par le
service). Pente douce, aucune rupture franche. En lisant le LIBELLÉ, la CAPACITÉ et surtout les
VERBATIMS de chaque circuit, on voit le même geste d'ensemble : Rémi ne fait pas un circuit puis un
autre — il combine plusieurs circuits sophistiqués dans une même création de solution. Sur la panne,
il ouvre plusieurs fronts (P4C15), conçoit pour tester (P4C8), génère des options (P4C1), explore des
angles inhabituels (P4C9), anticipe les imprévus (P4C10), réutilise l'acquis (P4C4) — le tout dans
la même réponse, les gestes s'appelant les uns les autres. C'est un plateau d'activation : tous les
circuits s'allument ensemble, à chaque fois.
→ UN SEUL bloc « très souvent » réunissant les six, dans l'ordre des totaux (5, 4, 3, 3, 3, 3).
PAS de bloc « souvent ». Forcer une coupure — isoler P4C15 seul, ou couper après P4C8 — inventerait
une rupture que les verbatims démentent. ⚠️ P4C8 se cite « total 4 », JAMAIS « total 3 » pour
l'aligner sur les circuits à 3 : les chiffres sont intangibles.
...
═══════════════════════════════════════════════════════════════════════
CHAMP 6 — synth_candidat (par bloc de fréquence, registre B)
...
FAIBLE + EN SOUTIEN : fusionnés dans le bloc « occasionnels » (rangés sur le total).
...
OCCASIONNELS Cécile P3 (FAIBLE + EN SOUTIEN fusionnés) :
...
TRÈS SOUVENT P5 Cécile (pilier entièrement sans cœur) :
...
═══════════════════════════════════════════════════════════════════════
CHAMP 7 — synth_rattachement (par bloc de fréquence)
...
FORMAT DIFFÉRENT SELON LE BLOC — règle absolue :

« très souvent » et « souvent » → "sont ce que le protocole nomme" :
...
── EXEMPLE ÉTALON « très souvent » Cécile P3 ──
...
ERREUR INTERDITE : utiliser "sont ce que le protocole nomme" pour les circuits du bloc « occasionnels ».
...
N'AFFICHER QUE LES BLOCS PRÉSENTS : si le pilier n'a pas de bloc « souvent » (plateau
total sans cassure, ou pilier à faible volume), ne pas écrire la section « souvent ».
...
remplace l'ancien seuil « amplitude ≥ 8 » de la doctrine 13.2, périmé) :
  ...
  - Si les gestes du mode partagent une même nature (composites, ou de haut niveau), le mode
    ...
    Exemple Rémi P4 (socle, plateau compact) : aucune cassure nette (totaux 5, 4, 3, 3, 3, 3). Les
    circuits de tête forment un plateau de gestes équivalents qui se combinent → le mode se construit
    sur le groupe de tête (P4C15 + les circuits combinatoires), reflétant la création multidimensionnelle.
...
"Ce mode découle de vos gestes dominants : vous assemblez plusieurs morceaux qui se complètent
(P4C15), vous concevez en gardant des tiroirs ouverts pour tester (P4C8), et vous générez des
options en explorant des angles inhabituels (P4C1, P4C9). Ces gestes ne se succèdent pas : vous les
activez ensemble dans la même création — vous ne planifiez pas une solution complète à l'avance,
vous la construisez en avançant, en combinant, en gardant des options prêtes. D'où « Intuitif et
...

{
  ...
  "echelle_classement": "total",  // IGNORÉ (vestige) : le rangement se fait TOUJOURS sur le total. Ne pas s'en servir.
...

{
  ...
  "blocs": [
    {
      "niveau": "très souvent",
      "synth_technique": "<Bloc très souvent : codes, cœur+total exacts, Facettes dominantes — registre A>",
    ...
    {
      "niveau": "souvent",
      "synth_technique": "<Bloc souvent : codes, cœur+total exacts — registre A>",
      "synth_candidat": "<M1 + M2 — registre B>",
      "synth_rattachement": "<souvent : sont ce que le protocole nomme>"
    },
    {
      "niveau": "occasionnels",
      "synth_technique": "<Bloc occasionnels : codes, cœur+total exacts ; EN SOUTIEN distingués — registre A>",
  ...
  "_note_blocs": "N'émettre que les blocs réellement présents (un pilier peut n'avoir que très souvent + occasionnels, ou très souvent seul + occasionnels). L'ordre est toujours très souvent → souvent → occasionnels.",
  "synthese_pilier": {
...
9.  vue_ensemble : 5 sections avec titres imposés mot pour mot.
    Titre 3ème section = "▸ Ce que vous faites occasionnellement, ou en appui".
...
24. (rectifié 23/06, méthode précisée 25/06) Rangement sur le TOTAL (jamais le cœur). Blocs :
    occasionnels (total 1-2, déjà posé par le service) ; très souvent / souvent attribués par TOI
    aux total ≥ 3 en jugeant la cassure SUR LE FOND (libellé + capacité + verbatims, comme le mode),
    sans forcer une cassure absente. Un profil compact dont les circuits se combinent dans une même
    démarche = un seul bloc « très souvent ». Écart cœur/total commenté SI marqué (≥3 ou total ≥ 2×cœur),
    sans reclasser.
...
29. (v11) INTANGIBILITÉ DES CHIFFRES : chaque total et chaque cœur cité dans synth_technique
    correspond EXACTEMENT au chiffre fourni dans l'entrée. Aucun chiffre réécrit pour homogénéiser
    un groupe ou justifier un rangement. (Erreur à ne pas refaire : P4C8 de Rémi, total 4, cité « 3 ».)
30. (v11) TRI INTERNE : dans chaque bloc, les circuits sont cités par total décroissant
    (le plus grand total en premier).
