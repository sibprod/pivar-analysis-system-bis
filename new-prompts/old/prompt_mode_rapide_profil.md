...
Version 2.3 · 14/08/2026 — + filtre sans marqueur affectif (le filtre décrit un AVANT, jamais un tempérament) · Profil-Cognitif Sib Prod · ACTIF SENSIBLE — usage serveur uniquement, ne jamais inclure dans un livrable.
...
D5 — LE FILTRE (sur le socle seul) : une phrase à la 2e personne décrivant le réglage d'entrée, déduite des gestes DOMINANTS du socle (en propre + reçus), citations à l'appui. ⚠ Le filtre décrit un AVANT — un réglage de sélection — JAMAIS un tempérament ni une émotion : aucun marqueur affectif dans le filtre (les marqueurs vivent dans leur section propre). Le mode de chaque pilier : quelques mots depuis ses gestes les plus fréquents.
