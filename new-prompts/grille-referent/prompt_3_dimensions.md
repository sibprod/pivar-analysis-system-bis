<!-- ⟦LOT 2026-09-11 ab⟧ prompt_3_dimensions.md — conditions d'exercice, jamais « ne pas attendre » -->
...
| `ne_pas_attendre` | ⭐ **RENOMMÉ 11/09/2026 — voir l'encadré ci-dessous.** Le champ garde son nom technique, mais son CONTENU change : ce sont désormais **les conditions dans lesquelles la capacité s'exerce**, jamais ce qu'il ne faut pas espérer |
...
## 🔴 CE QU'ON N'ÉCRIT PLUS — ARBITRAGE GARANTE, 11/09/2026

> *« On ne peut pas écrire "ne pas attendre" : c'est formulé comme une interprétation
> d'un comportement négatif anticipé. Donc non, interdit. »*

**Le champ `ne_pas_attendre` ne dit plus ce qui ne se produira pas.** Annoncer à un
référent ce qu'il ne doit pas espérer d'une personne, c'est projeter un comportement
futur à partir d'une mesure passée — une interprétation, pas un constat.

**Ce qu'il contient désormais : LES CONDITIONS D'EXERCICE.** Au présent, à l'indicatif,
et toujours tourné vers ce qui permet à la capacité de s'exprimer.

| ⛔ On n'écrit plus | ✅ On écrit |
|---|---|
| « Ne pas attendre d'anticipation spontanée en contexte calme » | « Cette anticipation s'exerce quand une contrainte externe structure la situation : urgence, délai, groupe à coordonner » |
| « Ne pas attendre de vue systémique en coordination ordinaire » | « Cette lecture des interdépendances s'exerce sous pression réelle, quand plusieurs fronts doivent tenir ensemble » |
| « Cette capacité disparaît sous urgence » | « Cette observation de soi s'exerce dans les temps calmes et organisés » |

**La règle** : on dit **ce dont le geste a besoin pour s'exercer**. Jamais ce qui
manquera, jamais ce qui risque d'arriver, jamais ce qu'il ne faut pas espérer.

Un référent lit alors une condition d'emploi — utile, actionnable — au lieu d'une
restriction sur une personne.
...
**DISTINCTION OBLIGATOIRE (ajout 04/09/2026, arbitrage garante — jurisprudence « MET Véronique ») :** « non établie » signifie NON MESURÉE (fenêtre insuffisante, test à venir). Une dimension MESURÉE dont le régime est absent MAIS dont la matière transmise porte un constat réel (synthèse, déclencheur « quand ça s’active ») N’EST PAS non établie : tu produis son bloc — `constat` (l’expression très ciblée, fidèle à la matière), `quand` (les contextes d’activation posés), `ne_pas_attendre` (ce que le régime absent interdit d’attendre) — sans un chiffre, sans un régime brut. Le référent a le droit de lire ce que la mesure a réellement montré, y compris quand elle a montré une expression rare.
...
3. Aucune dimension non établie (= NON MESURÉE) n'a été ajoutée — une mesurée-absente DOCUMENTÉE n'est pas « non établie » (distinction du 04/09/2026).
